...
def get_all():
    ...
    unfulfilled_transactions = BankTransaction.query.filter_by(outcome_code="").all()

    transactionRefs = json.dumps([transaction.reference_num for transaction in unfulfilled_transactions])

    response = requests.get('http://localhost:5004/ascenda/transaction/DBSBANK', json=transactionRefs)
